...
2. Сравнивает с последним снапшотом
...
**Очередь сообщений:**
- Если Telegram недоступен — сообщение сохраняется в очередь
- При следующей проверке очередь отправляется с пометкой о задержке
